...
SELECT COUNT(*) AS row_count 
FROM barbie_top_search_query;

SELECT COUNT(*) AS row_count 
FROM minecraft_movie_top_queries;

SELECT COUNT(*) AS row_count 
FROM supermario_bros_top_queries;

-- What are the top 5 most searched queries per movie?
SELECT query, `search interest` 
FROM barbie_top_search_query
ORDER BY `search interest` DESC 
LIMIT 5;
...
SELECT query, `search interest`, `increase percent` 
...
(case_id, 
 movie_title, 
 release_year, publicity_type, 
 attention_style,
 worldwide_revenue, 
 domestic_revenue, 
 budget, 
 reason_for_including, 
 data_reference)
VALUES
(1, 
 ...
 'Main positive-publicity example: movie became heavily tied to memes, 
                                   gaming culture, 
...
(2, 
 ...
 'Barbenheimer, 
  ...
  nostalgia, 
  memes, 
  event-style marketing',
  1448000000, 636800000, 145000000,
 'Strong positive-publicity example: movie became a cultural event through 
...
SELECT
    ...
    CASE p.movie_title
        WHEN 'Barbie' THEN (
            SELECT `barbie movie` 
            FROM barbie_trends_time_series 
            WHERE Time = '2023-07-01')
   
        WHEN 'The Super Mario Bros. Movie' THEN (
            SELECT `The Super Mario Bros. Movie` 
            FROM supermario_time_series 
            WHERE Time = '2023-04-01')
   
        WHEN 'A Minecraft Movie' THEN (
            SELECT `minecraft movie` 
            FROM minecraft_movie_time_series 
...
-- This measures how long the hype lasted, not just how big the peak was.
   SELECT 'Barbie' AS movie, 
      COUNT(*) AS months_above_10
   FROM barbie_trends_time_series 
   WHERE `barbie movie` > 10
UNION ALL
   SELECT 'Minecraft Movie', 
      COUNT(*)
   FROM minecraft_movie_time_series 
   WHERE `minecraft movie` > 10
UNION ALL
   SELECT 'Super Mario Bros Movie', 
      COUNT(*)
   FROM supermario_time_series 
   WHERE `The Super Mario Bros. Movie` > 10;
